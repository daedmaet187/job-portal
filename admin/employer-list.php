...
$query->execute();
 echo "<script>alert('Employer deleted');</script>"; 
...
if(isset($_GET['id']) && isset($_GET['action']))
{
    ...
    $sql="UPDATE tblemployers SET Is_Active=:active where id=:rid";
    $query=$dbh->prepare($sql);
    $query->bindParam(':rid',$rid,PDO::PARAM_STR);
    $query->bindParam(':active',$active,PDO::PARAM_STR);
    $query->execute();
    if($active == 1) {
        echo "<script>alert('Employer Activated');</script>"; 
    }else{
        echo "<script>alert('Employer Deactivated');</script>"; 
    }
    echo "<script>window.location.href = 'employer-list.php'</script>";
}
  ?>
                    ...
                    <div class="block">
                        ...
                        <div class="block-content block-content-full">
                            ...
                            <table class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
                                ...
                                <tbody>
                                    <?php
$sql="SELECT * from tblemployers ORDER BY tblemployers.Is_Active, tblemployers.id DESC";
$query = $dbh -> prepare($sql);
$query->execute();
...
{               ?>
                                    <tr>
                                        ...
                                        <td class="d-none d-sm-table-cell"><?php  echo htmlentities($row->RegDtae);?></td>
                                         ...
                                         <td class="d-none d-sm-table-cell"><a href="employer-list.php?action=<?php echo $row->Is_Active == 1? '0': '1' ?>&id=<?php echo $row->id ?>"><?php echo $row->Is_Active == 1? 'Deactivate': 'Activate' ?></a> | <a href="view-employer-details.php?viewid=<?php echo htmlentities ($row->id);?>"><i class="fa fa-eye" aria-hidden="true"></i></a> | <a href="employer-list.php?delid=<?php echo ($row->id);?>" onclick="return confirm('Do you really want to Delete ?');"><i class="fa fa-trash fa-delete" aria-hidden="true"></i></a></td>
                                    </tr>
                                    <?php $cnt=$cnt+1;}} ?> 
